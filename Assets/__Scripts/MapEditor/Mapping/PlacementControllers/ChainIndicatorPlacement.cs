    ...
    CMInput.INotePlacementActions
{
    ...
    protected override float GetContainerPosZ(ObjectContainer con)
    {
        if (con is ChainIndicatorContainer indicator)
        {
            ...
            {
                return (indicator.ParentChain.ChainData.SongBpmTime - Atsc.CurrentSongBpmTime) * EditorScaleController.EditorScale;
            }
            if (indicator.IndicatorType == IndicatorType.Tail)
            {
                return (indicator.ParentChain.ChainData.TailSongBpmTime - Atsc.CurrentSongBpmTime) * EditorScaleController.EditorScale;
            }
        }
